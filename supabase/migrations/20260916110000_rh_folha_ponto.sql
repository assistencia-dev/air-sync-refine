...
CREATE INDEX IF NOT EXISTS rh_ponto_audit_employee_idx ON public.rh_ponto_audit(employee_id, created_at DESC);
...
CREATE POLICY rh_ponto_records_self_select ON public.rh_ponto_records FOR SELECT TO authenticated USING (
  employee_id IN (SELECT e.id FROM public.rh_employees e WHERE e.ponto_portal_user_id = (SELECT u.id FROM public.users u WHERE u.auth_id = auth.uid()) AND e.ponto_access_enabled = true)
  OR (SELECT u.role_key FROM public.users u WHERE u.auth_id = auth.uid()) IN ('SUPER_ADMIN', 'ADMIN_OPERACIONAL')
);
CREATE POLICY rh_ponto_records_self_insert ON public.rh_ponto_records FOR INSERT TO authenticated WITH CHECK (
  employee_id IN (SELECT e.id FROM public.rh_employees e WHERE e.ponto_portal_user_id = (SELECT u.id FROM public.users u WHERE u.auth_id = auth.uid()) AND e.ponto_access_enabled = true)
  OR (SELECT u.role_key FROM public.users u WHERE u.auth_id = auth.uid()) IN ('SUPER_ADMIN', 'ADMIN_OPERACIONAL')
);
CREATE POLICY rh_ponto_records_admin_update ON public.rh_ponto_records FOR UPDATE TO authenticated
  USING ((SELECT u.role_key FROM public.users u WHERE u.auth_id = auth.uid()) IN ('SUPER_ADMIN', 'ADMIN_OPERACIONAL'))
  WITH CHECK ((SELECT u.role_key FROM public.users u WHERE u.auth_id = auth.uid()) IN ('SUPER_ADMIN', 'ADMIN_OPERACIONAL'));
CREATE POLICY rh_ponto_audit_admin_select ON public.rh_ponto_audit FOR SELECT TO authenticated
  USING ((SELECT u.role_key FROM public.users u WHERE u.auth_id = auth.uid()) IN ('SUPER_ADMIN', 'ADMIN_OPERACIONAL'));
CREATE POLICY rh_ponto_audit_admin_insert ON public.rh_ponto_audit FOR INSERT TO authenticated
  WITH CHECK ((SELECT u.role_key FROM public.users u WHERE u.auth_id = auth.uid()) IN ('SUPER_ADMIN', 'ADMIN_OPERACIONAL'));
