...
export function TabValePassagem() {
  ...
  useEffect(() => {
    listRhEmployees({ data: { benefit_type: 'passagem' } }).then((data) => setEmployees(data as Employee[])).catch((err) => setError(err instanceof Error ? err.message : 'Erro ao carregar dados VT')).finally(() => setIsLoading(false));
  }, []);
  ...
  if (isLoading) return <div className="flex items-center justify-center py-12"><Loader2 className="w-6 h-6 animate-spin" style={{ color: '#3B82F6' }} /><span className="ml-3" style={{ color: '#94A3B8' }}>Carregando dados de Vale Passagem...</span></div>;
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><CreditCard className="w-8 h-8" style={{ color: '#3B82F6' }} /><div><h2 className="text-2xl font-bold text-white">Vale Passagem (VT)</h2><p className="text-sm mt-1 text-slate-400">Dias trabalhados no mês × valor diário salvo no Supabase.</p></div></div><input type="month" value={selectedYear + '-' + String(selectedMonth).padStart(2, '0')} onChange={(event) => { const parts = event.target.value.split('-').map(Number); setSelectedYear(parts[0]); setSelectedMonth(parts[1]); }} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white" /></div>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><Summary label="Colaboradores VT" value={String(employees.length)} color="#3B82F6" /><Summary label={'Dias trabalhados (' + selectedMonth + '/' + selectedYear + ')'} value={String(daysWorked)} color="#3B82F6" /><Summary label="Total VT no mês" value={formatCurrencyBRL(totalMonthly)} color="#F59E0B" /></div>
    <div className="rounded-lg border border-slate-700 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-slate-800 text-left text-xs uppercase text-slate-300"><th className="px-6 py-3">Colaborador</th><th className="px-6 py-3">Valor diário</th><th className="px-6 py-3">Dias trabalhados</th><th className="px-6 py-3">Total VT</th></tr></thead><tbody className="divide-y divide-slate-700">{employees.map((emp) => { const daily = emp.fare_cents / 100; return <tr key={emp.id}><td className="px-6 py-4"><p className="font-semibold text-white">{emp.full_name}</p><p className="text-xs mt-1 text-slate-500">{emp.unit}</p></td><td className="px-6 py-4 text-slate-300">{formatCurrencyBRL(daily)}</td><td className="px-6 py-4 text-slate-300">{daysWorked}</td><td className="px-6 py-4 font-bold text-amber-400">{formatCurrencyBRL(daily * daysWorked)}</td></tr>; })}</tbody></table>{!employees.length && <p className="p-8 text-center text-xs text-slate-400">Nenhum colaborador de Vale Passagem cadastrado.</p>}</div>
  </div>;
}
