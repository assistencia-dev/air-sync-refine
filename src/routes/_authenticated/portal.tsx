...
import { LogOut, PlusCircle, RefreshCw, Ticket } from "lucide-react";
...
function PortalPage() {
  ...
  const mut = useMutation({
    mutationFn: (input: { occurrence_type: string; description: string }) => createTicket({ data: input }),
    onSuccess: (t) => {
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'Inter',system-ui,sans-serif" }}>
      <header className="bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-slate-500 hidden md:inline">Portal do Cliente</span>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <section className="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 shadow-sm h-fit">
          ...
          <p className="text-xs text-slate-500 mb-6">Endereço e dados da unidade herdados automaticamente do seu cadastro.</p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-1.5 text-slate-800">Tipo de ocorrência</label>
              <select value={occurrence} onChange={(e) => setOccurrence(e.target.value)} className="w-full px-3 py-2.5 rounded-md border border-slate-300 text-sm">
                {OCCURRENCE_TYPES.map((t) => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider mb-1.5 text-slate-800">Descrição (mín. 20 caracteres)</label>
              <textarea
                required
                minLength={20}
                value={desc}
        ...
        <section className="lg:col-span-3 bg-white rounded-xl border border-slate-200 shadow-sm">
          <div className="p-6 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2"><Ticket className="w-5 h-5" style={{ color: "#0284C7" }} /> Meus chamados</h2>
              <p className="text-xs text-slate-500 mt-0.5">Apenas os chamados abertos por você.</p>
            </div>
